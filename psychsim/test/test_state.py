...
def test_max_size_certainty():
    test_max_size(2)


def test_max_size_joint():
    test_max_size(3)


def test_max_size(max_size=4, num_iterations=1000):
    for i in range(num_iterations):
        s = make_state(num_splits=1, max_uncertainty=5)
        old = sorted([(world, prob) for world, prob in s.worlds()], 
        ...
        for world, prob in old:
            if world in now.domain():
                assert prob >= floor-1e-8, f'{prob} < {floor}\n{world}'
                break
            else:
                assert prob <= floor+1e-8, f'{prob} > {floor}\n{world}'
